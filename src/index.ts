export class El extends HTMLElement {
  static els: Record<string, any> = {};
  static stash: Record<string, any> = {};
  ...
  private _cache = { d: {}, clear: () => (this._cache.d = {}) };
  ...
  _unstash() {
    ...
    const { _contextId } = El;
    El._contextId = this._id;
    const allElement = [...(this.shadowRoot || this).querySelectorAll('*'), this]
    for (const el of allElement){
      for (const attr of el.attributes) {
        const attrName = camel(attr.name);
        if (attr.value in El.stash) {
          (el as any)[attrName] = El.stash[attr.value]
        } else if (attr.name in (el as any).__proto__) {
        } else {
          try {
            (el as any)[attrName] = attr.value
          } catch {
          }
        }
      }
  ...
  get $refs() {
    return new Proxy(
      {},
      { get: (obj, key: string) => this.shadowRoot!.querySelector(`[ref="${key}"]`) },
    );
  }
  ...
  static dep(path) {
    (El.dep as Record<string, any>)._path = !El._contextId && path;
  ...
  static morph(l, r) {
    let ls = 0; let rs = 0; let le = l.childNodes.length; let re = r.childNodes.length
    const lc = [...l.childNodes]; const
      rc = [...r.childNodes];
    const content = e =>
  ...
  static zcss(...args) {
    let lines = []; let stack: any[] = []; let open; let opened; let close
    ...
    for (let line of src.split(/\n/)) {
      ...
      if (line.match(/^\s*@[msdk].*\{/))
        ...
        close = (!stack.pop() && lines.push('}')) | 1;
      else {
        if (!line.trim()) {continue};
        if (opened && (open || close)) {opened = close = lines.push('}') & 0};
        if (open || close)
          opened = !(open =
            lines.push(stack.join` `.replace(/ &/g, '') + '{') & 0);
        lines.push(line);
      }
    }
